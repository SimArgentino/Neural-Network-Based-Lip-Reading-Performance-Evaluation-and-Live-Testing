...
# Load the predictor
predictor = dlib.shape_predictor(r"C:\Users\simon\Desktop\prova\Computer-Vision-Lip-Reading-2.0\model\face_weights.dat")
...
# Counter for numbering the data collected
data_count = 1

# List of words available for data collection
words = ["ciao", "ok", "demo", "uova", "cane", "gatto", "pizza", "pasta", "auto", "moto"]
options = ", ".join(words)

label = input("What word you like to collect data for? The options are \n" + options + ": ")
labels = []

# Ask the user for a custom lip distance threshold if any
custom_distance = input("If you want, enter a custom lip distance threshold or -1: ")

# Ask if the user wants to clean the output directory of the current word
...
if clean_output_dir == "yes":
    ...
    outputs_dir = r"C:\Users\simon\Desktop\prova\Computer-Vision-Lip-Reading-2.0\collected_data\outputs"
...
# If the user provides a valid custom lip distance, use it as the threshold
...
# Main loop to process each frame from the video capture    
while True:
    ...
    for face in faces:
        ...
        if(determining_lip_distance != 0 and LIP_DISTANCE_THRESHOLD != None):
            ...
            # Apply CLAHE to enhance the L channel (lightness)
            ...
            if lip_distance > LIP_DISTANCE_THRESHOLD: # Check if person is talking
                # Display "Talking" text on the frame
                cv2.putText(frame, "Talking", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
                 # Store current lip frames and reset not talking counter
                curr_word_frames += [lip_frame.tolist()]
                not_talking_counter = 0

                 # Indicate recording of word is in progress
                cv2.putText(frame, "RECORDING WORD RIGHT NOW", (50, 100), cv2.FONT_HERSHEY_SIMPLEX, 1, ORANGE, 2)

            else:
                 # Display "Not talking" text on the frame
                cv2.putText(frame, "Not talking", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, RED, 2)
                # Increment not talking counter
                not_talking_counter += 1
                
               # Check conditions for stopping or continuing word recording
                if not_talking_counter >= NOT_TALKING_THRESHOLD and len(curr_word_frames) + PAST_BUFFER_SIZE == TOTAL_FRAMES: 
                    ...
                    # Prepare to save current word frames
                    data_count += 1
                ...
                # curr word frames not fully done yet, add ending buffer frames
                     ...
                     # Continue recording word frames
                    cv2.putText(frame, "RECORDING WORD RIGHT NOW", (50, 100), cv2.FONT_HERSHEY_SIMPLEX, 1, ORANGE, 2)
                ...
                # too little frames, discard the data
                    ...
                    # Discard current frames due to insufficient data or excess frames
                    cv2.putText(frame, "NOT RECORDING WORD", (50, 100), cv2.FONT_HERSHEY_SIMPLEX, 1, BLUE, 2)
                 ...
                 # Store past frames in a circular buffer
            ...
            # Update list of lip distances for calibration
            lip_distances.append(distance)
            # Update lip distance threshold after calibration phase
            if(determining_lip_distance == 0):
                LIP_DISTANCE_THRESHOLD = sum(lip_distances) / len(lip_distances) + 2

    # Display total collected words and exit instructions
...
def saveAllWords(all_words):
    ...
    """
    output_dir = r"C:\Users\raf19\Desktop\Profetto2\Computer-Vision-Lip-Reading-2.0-main\Computer-Vision-Lip-Reading-2.0-main\collected_data"
    # Initialize a counter to keep track of the next available directory number
    next_dir_number = 1
    # Loop through each word's frames in all_words
    for i, word_frames in enumerate(all_words):
        # Get the corresponding label for the current word from the labels list
        label = labels[i]
        # Construct the path for the folder to store the current word's frames
        word_folder = os.path.join(output_dir, label + "_" + f"{next_dir_number}")
        # Check if a folder with the same name already exists, if so, increment the directory number
        while os.path.exists(word_folder):
            next_dir_number += 1
            word_folder = os.path.join(output_dir, label + "_" + f"{next_dir_number}")
        
        # Create the directory for the current word's frames
        os.makedirs(word_folder)
         # Define the path to save the frame data in a text file (JSON format)
        txt_path = os.path.join(word_folder, "data.txt")

         # Save the frame data as JSON in the text file
        with open(txt_path, "w") as f:
            f.write(json.dumps(word_frames))

        images = []
        # Loop through each frame of the word (word_frames is a list of image data)
        for j, img_data in enumerate(word_frames):
            # Create a new RGB image with the same dimensions as img_data
            img = Image.new('RGB', (len(img_data[0]), len(img_data)))
            # Load the image to access its pixel data
            pixels = img.load()
             # Loop through each pixel in img_data to populate the new image
            for y in range(len(img_data)):
                for x in range(len(img_data[y])):
                    # Convert the pixel data from img_data to an RGB tuple and set it in the new image
                    pixels[x, y] = tuple(img_data[y][x])
            # Define the path to save the current image frame as a PNG file
            img_path = os.path.join(word_folder, f"{j}.png")
            img.save(img_path)
            # Read the saved image file and append it to the images list (to create a video later)
